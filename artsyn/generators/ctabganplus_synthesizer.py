...
from torch.nn import (Dropout, LeakyReLU, Linear, Module, ReLU, Sequential, Conv2d, ConvTranspose2d, Sigmoid, init,
                      BCELoss, CrossEntropyLoss, SmoothL1Loss, LayerNorm)
from artsyn.generators.ctabganplus_transformer import ImageTransformer, DataTransformer
...
def determine_layers_disc(side, num_channels):
    ...
    for _ in range(len(layer_dims) - 1):
        layer_norms.append([int(num_c), int(num_s), int(num_s)])
        num_c = num_c * 2
        num_s = num_s / 2

    layers_d = []

    for prev, curr, ln in zip(layer_dims, layer_dims[1:], layer_norms):
        layers_d += [
...
def determine_layers_gen(side, random_dim, num_channels):
    ...
    layers_g = [ConvTranspose2d(random_dim, layer_dims[-1][0], layer_dims[-1][1],
                                1, 0, output_padding=0, bias=False)]

    for prev, curr, ln in zip(reversed(layer_dims), reversed(layer_dims[:-1]), layer_norms):
        layers_g += [LayerNorm(ln), ReLU(True), ConvTranspose2d(prev[0], curr[0], 4,
                                                                2, 1, output_padding=0, bias=True)]
    return layers_g
...
def calc_gradient_penalty_slerp(net_d, real_data, fake_data, transformer, device='cpu', lambda_=10):
    ...
    disc_interpolates, _ = net_d(interpolates)
...
class CTABGANPlusSynthesizer:
    def __init__(self, metadata, class_dim=(256, 256, 256, 256), random_dim=100, num_channels=64, l2scale=1e-5,
                 batch_size=500, epochs=150, random_state=None):
        ...
        self.p_type = {}
        self.categorical = []
        self.non_categorical = []
        col = 0
        for col in metadata.columns.keys():
            if metadata.columns[col]["sdtype"] == "numerical":
                self.non_categorical.append(col)
            elif metadata.columns[col]["sdtype"] == "categorical":
                self.categorical.append(col)

        # If the last column is categorical
        if metadata.columns[col]["sdtype"] == "categorical":
            self.p_type = { "classification": str(col)}

    def fit(self, train_data=pd.DataFrame):
        problem_type = None
        target_index = None
        if self.p_type:
            problem_type = list(self.p_type.keys())[0]
            if problem_type:
                target_index = train_data.columns.get_loc(self.p_type[problem_type])

        self.transformer = DataTransformer(train_data=train_data, categorical_list=self.categorical,
                                           mixed_dict=self.mixed, general_list=self.general,
                                           non_categorical_list=self.non_categorical, random_state=self._random_state)
        ...
        for _ in tqdm(range(self.epochs), desc="CTABGAN++ Training  "):
            for id_ in range(steps_per_epoch):
                ...
                for _ in range(ci):
                    ...
                    pen = calc_gradient_penalty_slerp(discriminator, real_cat, fake_cat,  self.Dtransformer,
                                                      self.device)
                    pen.backward()
                    optimizerD.step()
