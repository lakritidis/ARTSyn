...
from torch.nn import (Dropout, LeakyReLU, Linear, Module, ReLU, Sequential,
                      Conv2d, ConvTranspose2d, BatchNorm2d, Sigmoid, init, BCELoss, CrossEntropyLoss)
from artsyn.generators.ctabgan_transformer import ImageTransformer, DataTransformer
...
class Condvec(object):
    ...
    5) p_log_sampling -> list containing log of probability mass distribution of categories within their respective
    one-hot-encoding representations
    6) p_sampling -> list containing probability mass distribution of categories within their respective
    one-hot-encoding representations
    ...
    def __init__(self, data, output_info):
        ...
        for item in output_info:
            ...
            if item[1] == 'tanh':
                ...
                # using starting (st) and ending (ed) position of any given one-hot-encoded representation to obtain
    ...
    def sample_train(self, batch):
        ...
        # each conditional vector in vec is a one-hot vector used to highlight a specific category across all
        # possible one-hot-encoded representations
        ...
        # matrix of shape (batch x total no. of one-hot-encoded representations) with 1 in indexes of chosen
        # representations and 0 elsewhere
    ...
    def sample(self, batch):
        ...
        # each conditional vector in vec is a one-hot vector used to highlight a specific category across all possible
...
def cond_loss(data, output_info, c, m):
    """
    Used to compute the conditional loss for ensuring the generator produces the desired category as specified by the
    conditional vector.
    ...
    for item in output_info:
        ...
        if item[1] == 'tanh':
            st += item[0]
            continue
        # compute cross entropy loss between generated one-hot-encoding and corresponding encoding of conditional vector
        elif item[1] == 'softmax':
            ed = st + item[0]
            ed_c = st_c + item[0]
...
def get_st_ed(target_col_index, output_info):
    ...
    # obtain the ending position by using the dimension size of the one-hot-encoding used to represent the target column
...
class Classifier(Module):
    ...
    def __init__(self, input_dim, class_dims, st_ed):
        ...
        # subtract the target column size from the input dimensionality
        self.dim = input_dim - (st_ed[1] - st_ed[0])

        # store the starting and ending positons of the target column in the input data
        self.str_end = st_ed

        # build the layers of the network with same hidden layers as discriminator
        seq = []
        tmp_dim = self.dim
        ...
        # in case of binary classification the last layer outputs a single numeric value which is squashed to a
        # probability with sigmoid
        if (st_ed[1] - st_ed[0]) == 2:
            seq += [Linear(tmp_dim, 1), Sigmoid()]

        # in case of multi-classs classification, the last layer outputs an array of numeric values associated to each
        # class
        else:
...
def determine_layers_disc(side, num_channels):
    """
    This function describes the layers of the discriminator network as per DCGAN
    ...
    while layer_dims[-1][1] > 3 and len(layer_dims) < 4:
        # the number of channels increases by a factor of 2 whereas the height/width decreases by the same factor with
        # each layer
        layer_dims.append((layer_dims[-1][0] * 2, layer_dims[-1][1] // 2))

    # constructing the layers of the discriminator network based on the recommendations mentioned in
    ...
    # last layer reduces the output to a single numeric value which is squashed to a probability using sigmoid function
    layers_d += [
...
def determine_layers_gen(side, random_dim, num_channels):
    ...
    # similarly constructing the layers of the generator network based on the recommendations mentioned in
    # https://arxiv.org/abs/1511.06434
    # first layer of the generator takes the channel dimension of the noise matrix to the desired maximum channel size
    # of the generator's layers
    layers_g = [
        ConvTranspose2d(
    ...
    # such as the no. of channels reduce by a factor of 2 and height/width of generated image increases by the same
    # factor with each layer
...
class CTABGANSynthesizer:
    ...
    2) fit() -> takes the pre-processed training data and associated parameters as input to fit the CTABGAN model
    3) sample() -> takes as input the no. of data rows to be generated and synthesizes the corresponding no of data rows
    """

    def __init__(self, class_dim=(256, 256, 256, 256), random_dim=100, num_channels=64, l2scale=1e-5, batch_size=500,
                 epochs=1, random_state=None):
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, p_type={}):
        ...
        # obtaining the desired height/width for converting tabular data records to square images for feeding it to
        # discriminator network
        sides = [4, 8, 16, 24, 32]
        # the discriminator takes the transformed training data concatenated by the corresponding conditional vectors
        # as input
        ...
        # obtaining the desired height/width for generating square images from the generator network that can be
        # converted back to tabular domain
        sides = [4, 8, 16, 24, 32]
        col_size_g = data_dim
        for i in sides:
            if i * i >= col_size_g:
        ...
        optimizer_g = Adam(self.generator.parameters(), **optimizer_params)
        optimizer_d = Adam(discriminator.parameters(), **optimizer_params)

        st_ed = None
        classifier = None
        optimizer_c = None
        if target_index is not None:
            # obtain the one-hot-encoding starting and ending positions of the target column in the transformed data
            st_ed = get_st_ed(target_index, self.transformer.output_info)

            # configure the classifier network and it's optimizer accordingly
        ...
        # initialize the image transformer objects for the generator and discriminator networks for transitioning
        # between image and tabular domain
        ...
        for _ in tqdm(range(self.epochs)):
            for _ in range(steps_per_epoch):
                ...
                # concatenating conditional vectors and converting resulting noise vectors into the image domain to be
                # fed to the generator as input
                noisez = torch.cat([noisez, c], dim=1)
                noisez = noisez.view(self.batch_size, self.random_dim + self.cond_generator.n_opt, 1, 1)

                # sampling real data according to the conditional vectors and shuffling it before feeding to
                # discriminator to isolate conditional loss on generator
                ...
                # applying final activation on the generated data (tanh for numeric and gumbel-softmax for categorical)
                ...
                # computing the probability of the discriminator to correctly classify real samples hence y_real should
                # ideally be close to 1
                y_real, _ = discriminator(real_cat_d)

                # computing the probability of the discriminator to correctly classify fake samples hence y_fake should
                # ideally be close to 0
                y_fake, _ = discriminator(fake_cat_d)

                # computing the loss to essentially maximize the log likelihood of correctly classifiying real and fake
                # samples as log(D(x))+log(1−D(G(z))) or equivalently minimizing the negative of
                # log(D(x)) + log(1−D(G(z))) as done below
                loss_d = (-(torch.log(y_real + 1e-4).mean()) - (torch.log(1. - y_fake + 1e-4).mean()))

                # accumulate gradients based on the loss
                loss_d.backward()

                # compute the backward step to update weights of the discriminator
                optimizer_d.step()
                ...
                # compute the conditional loss to ensure the generator generates data records with the chosen category
                # as per the conditional vector
                cross_entropy = cond_loss(faket, self.transformer.output_info, c, m)

                # compute the loss to train generator where we want y_fake to be close to 1 to fool the discriminator
                ...
                # independently we may use retain_graph=True in backward() method in the first back-propagated loss
                # to maintain the computation graph to execute the second backward pass efficiently
                g.backward(retain_graph=True)

                # compute the information loss by comparing means and stds of real/fake feature representations
                # extracted from discriminator's penultimate layer
                ...
                if problem_type:
                    ...
                    # computing classifier's target column predictions on the fake data along with returning
                    # corresponding true labels
    ...
    def sample(self, n):
        ...
        # applying the inverse transform and returning synthetic data in a similar form as the original pre-processed
        # training data
        result = self.transformer.inverse_transform(data)

        return result[0:n]

    def sample_remaining_columns(self, max_tries_per_batch=500, known_columns=None):
        if known_columns is None:
            print("CTABGAN cannot sample_remaining_columns without known_columns")
            exit()

        requested_samples = known_columns.shape[0]
        ...
        while num_samples < requested_samples:
            ...
            for i in range(num_generated_samples):
                for j in range(known_columns.shape[1]):
                    ref_data = known_columns.iloc[:, j].to_numpy()
                    # print(ref_data[i], np.round(generated_data[i, int(known_columns.columns[j])]))
